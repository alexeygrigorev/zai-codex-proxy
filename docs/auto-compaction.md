...
Some clients use an automatic compaction threshold to keep prompts under a model context limit. When that threshold is
misconfigured, the client can get stuck on repeated "context too long" errors.
...
Auto-compaction is implemented for the Z.AI-only proxy path. On context-length errors, codex-proxy performs a
**summary compaction** via the configured compaction target and retries with a synthetic system message containing a
summary.
