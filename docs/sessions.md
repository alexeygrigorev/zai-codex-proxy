...
codex-proxy uses sticky routing to keep repeated requests on the same upstream account/model in order to maximize
...
This document describes the **session key** mechanism used to stabilize sticky routing across rewrites.
...
OpenAI Responses supports `previous_response_id` for linking turns. Z.AI does not need to implement it for the proxy to
use it as a local routing hint.
