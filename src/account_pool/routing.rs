    ...
    pub fn route(
        ...
        if let Some(binding) = state.sticky_bindings.read().get(&cache_key).cloned() {
            for candidate in candidates {
                if candidate.upstream_model != binding.model
                    ...
                    && snapshot.alive
                {
                    ...
                    debug!(
                        ...
                        cache_key, account.id, candidate.upstream_model
                    );
...
mod tests {
    ...
    use crate::codex_wire::schema::responses_wire::ChatMessage;
    ...
    fn candidate(priority_index: usize, model: &str) -> RouteCandidate {
        RouteCandidate {
    ...
    fn prefers_first_healthy_candidate_in_order() {
        ...
        pool.load_accounts(vec![
            account("zai-1", Some(vec!["glm-4.6"]), 1),
            account("zai-2", Some(vec!["glm-4.6"]), 1),
        ]);
        pool.mark_success(0);
        pool.mark_success(1);
        ...
        let route = Router::resolve_route(
            &pool,
            &state,
            &[candidate(0, "glm-4.6"), candidate(1, "glm-4.6")],
            &messages(),
            None,
        )
        .unwrap();
        assert_eq!(route.upstream_model, "glm-4.6");
    }
