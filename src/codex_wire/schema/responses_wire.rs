...
pub struct InputItem {
    ...
    // Responses wire compaction artifact.
    #[serde(default)]
...
pub struct Tool {
    ...
    // web_search config in the Codex Responses wire shape.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub web_search: Option<WebSearchConfig>,

    // Responses API namespace tools contain child function tools here.
    ...
    for message in messages {
        ...
        if role == "tool" {
            out.push(InputItem {
                ...
                changes: None,
                output: message.content.as_ref().map(chat_content_to_content),
                stdout: None,
                stderr: None,
        ...
        out.push(InputItem {
            ...
            recipient: None,
            name: message.name.clone(),
            content: message.content.as_ref().map(chat_content_to_content),
...
fn content_to_text(content: &Content) -> String {
    match content {
        ...
        Content::Parts(parts) => parts
            .iter()
            .filter_map(|part| part.text.as_deref().or(part.encrypted_content.as_deref()))
