...
pub struct RequestUsageStart<'a> {
...
impl UsageStore {
    fn record_request_start_at(&self, now: u64, minute_ts: u64, request: RequestUsageStart<'_>) {
        let is_error = !request.status.is_success();
        ...
        if let Some(key_id) = request.key_id {
            ...
            entry.request_bytes += request.request_bytes;
            entry.last_seen_at = Some(now);
        }

        let mut guard = self.by_account.write();
        let k = request.account_id.to_string();
        ...
        entry.request_bytes += request.request_bytes;
        entry.last_seen_at = Some(now);
        *entry.by_model.entry(request.model.to_string()).or_insert(0) += 1;
    ...
    pub fn record_request_start_handle(&self, request: RequestUsageStart<'_>) -> UsageHandle {
        let now = now_secs();
        let minute_ts = now - (now % BASE_BUCKET_SECONDS);
        let key_id = request.key_id.map(|v| v.to_string());
        let account_id = request.account_id.to_string();
        self.record_request_start_at(now, minute_ts, request);
        UsageHandle {
            minute_ts,
            key_id,
            account_id,
    ...
    pub fn record_response_bytes(&self, handle: &UsageHandle, response_bytes: u64) {
        ...
        let mut guard = self.by_account.write();
        if let Some(entry) = guard.get_mut(&handle.account_id) {
            entry.response_bytes += response_bytes;
            entry.last_seen_at = Some(now);
        }
