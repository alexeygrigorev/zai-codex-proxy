...
use crate::codex_wire::schema::sse::{
    ...
    ResponseOutputItemDoneData, ResponseOutputTextDeltaData, Usage,
};
...
pub fn stream_responses_sse(
    ...
    Box::pin(async_stream::stream! {
        ...
        loop {
            ...
            while let Some(nl) = line_buf.find('\n') {
                ...
                for tc_delta in delta.tool_calls {
                    ...
                    // The tool name arrives in the first delta, so by the time we
                    // announce the item we know whether Codex expects a
                    // `custom_tool_call` (apply_patch), a shell call, or a plain
                    // function call. Emit the correct type up front so the `added`
                    // and `done` events agree.
                    if is_new {
                        entry.item = initial_tool_item(&entry.name, &entry.call_id, &custom_names);
                        yield Ok(encode_event(
                            &mut seq_num,
        ...
        final_resp.usage = Some(final_usage.unwrap_or(Usage {
            input_tokens: 0,
            output_tokens: 0,
            total_tokens: 0,
            input_tokens_details: None,
...
impl MessageState {
    fn set_text(&mut self, full_text: &str) {
        if let OutputItem::Message(m) = &mut self.item
            && let Some(OutputContentPart::OutputText { text }) = m.content.first_mut()
        {
...
fn finalize_tool_call(
    ...
    if custom_names.contains(&name) {
        ...
        OutputItem::FunctionCall(FunctionCallItem {
            id: call_id.clone(),
            status: "in_progress".into(),
            namespace: tool_namespace_for_name(req, &name),
            name,
            arguments: args,
            call_id: call_id.clone(),
            thought_signature: None,
...
/// Build the in-progress output item announced in `response.output_item.added`.
/// Mirrors the final item type chosen by `finalize_tool_call` so streaming
/// clients see a consistent type for a given output index.
fn initial_tool_item(name: &str, call_id: &str, custom_names: &HashSet<String>) -> OutputItem {
    if custom_names.contains(name) {
        OutputItem::CustomToolCall(CustomToolCallItem {
...
fn tool_namespace_for_name(req: &ChatRequest, name: &str) -> Option<String> {
    ...
    for tool in &req.tools {
        ...
        match (&namespace, &tool.namespace) {
            (None, Some(candidate)) => namespace = Some(candidate.clone()),
...
fn set_item_status(item: &mut OutputItem, status: &str) {
    match item {
        ...
        OutputItem::CustomToolCall(c) => c.status = status.to_string(),
    }
}
...
mod tests {
    use super::*;
    use crate::codex_wire::schema::responses_wire::{ChatMessage, ChatRequest};
    ...
    fn test_request() -> ChatRequest {
        ChatRequest {
            ...
            messages: vec![ChatMessage {
                role: "user".into(),
                content: Some(
                    crate::codex_wire::schema::responses_wire::ChatContent::Text("hello".into()),
                ),
                name: None,
                tool_call_id: None,
                tool_calls: Vec::new(),
    ...
    #[tokio::test]
    async fn stream_skips_malformed_chunks_and_completes_valid_output() {
        let upstream = futures::stream::iter(vec![
            Ok(Bytes::from("data: {not json}\n\n")),
            Ok(Bytes::from(
                r#"data: {"choices":[{"delta":{"content":"hello"}}],"usage":{"prompt_tokens":2,"completion_tokens":3}}"#,
            )),
            Ok(Bytes::from("\n\n")),
            Ok(Bytes::from("data: [DONE]\n")),
        ]);

        let chunks = stream_responses_sse(upstream, "glm-5.2", 123, &test_request(), 1)
            .collect::<Vec<_>>()
            .await;
    ...
    #[tokio::test]
    async fn stream_emits_custom_tool_call_for_apply_patch() {
        ...
        let upstream = futures::stream::iter(vec![
            Ok(Bytes::from(
                r#"data: {"choices":[{"index":0,"delta":{"tool_calls":[{"index":0,"id":"call_1","type":"function","function":{"name":"apply_patch","arguments":"{\"input\":\"*** Begin Patch\"}"}}]}}]}"#,
            )),
            Ok(Bytes::from("\n\n")),
            Ok(Bytes::from("data: [DONE]\n")),
        ]);
        ...
        assert!(
            body.contains("custom_tool_call"),
            "missing custom_tool_call in: {body}"
        );
